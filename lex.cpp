...
static bool is_ws(u8 ch) { return ch == Tab || ch == Space; }

static const std::map<ptr, Kw_e> kws{{hash("fn"), Fn}};
...
SourceLocation::SourceLocation(FSFile &file)
    : file(file), line(1), col(1), indent(0), it(file.code.begin()),
      end(file.code.end()) {}
