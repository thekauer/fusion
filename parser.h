...
struct FnDecl : AstExpr {
    ...
    FnDecl(std::unique_ptr<FnProto> proto,std::vector<std::unique_ptr<AstExpr>>&& body) : 
    AstExpr(AstType::FnDecl),proto(move(proto)),body(move(body)){};
    void print_name() override {std::cout <<"FnDecl: " << proto->name.getName()<<"\n";}
    llvm::Value* codegen(FusionCtx& ctx) override;
    void pretty_print() override;
};
...
struct FnCall : AstExpr {
    ...
    FnCall(Token name,std::vector<std::unique_ptr<AstExpr>>&& args): AstExpr(AstType::FnCall),name(name),args(std::move(args)){};
