...
public:
    ...
    ClassStmt(const SourceLocation& sl, std::unique_ptr<VarExpr>&& name, std::unique_ptr<Body>&& body) : AstExpr(AstType::ClassStmt, sl), body(std::move(body)), name(std::move(name)) {
        ty = get_class_type();
    };
    llvm::Value* codegen(FusionCtx& ctx) const override;
    void pretty_print() const override;
